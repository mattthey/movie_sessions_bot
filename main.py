...
def get_buttons(current_position=0):
    ...
    if current_position > 0:
        result.append(InlineKeyboardButton(text="Назад", callback_data=str(current_position - 1)))
    if current_position < len(FILMS):
        result.append(InlineKeyboardButton(text="Далее", callback_data=str(current_position + 1)))
    return result
